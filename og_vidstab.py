...
INPUTS: VIDEO FILE
OUTPUTS: STABILIZED VIDEO AVI, TRANSFORMATION DF CSV, SMOOTHED TRAJECTORY CSV
...
ap.add_argument("-c", "--compareOutput", default=0, type=int, help="Should output be a side by side comparison")
...
for k in range(N - 1):
    ...
    # calc flow of movement resource:
    ...
    T = cv2.estimateAffinePartial2D(prev_corner2, cur_corner2)[0]
    # translation x
    dx = T[0, 2]
    # translation y
    dy = T[1, 2]
...
for k in range(N - 1):
    ...
    if args['compareOutput'] > 0:
        # resize to max width (if current width larger than max width)
